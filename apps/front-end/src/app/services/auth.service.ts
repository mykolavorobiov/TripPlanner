...
export class AuthService {
  ...
  constructor() {
    localStorage.removeItem('trip-planner-session');
  ...
  private async loadInitialSession(): Promise<void> {
    const oauthSession = await this.exchangeOAuthCallback();
    if (oauthSession) {
      this.setSession(oauthSession);
      return;
    }
    try {
  ...
  private async exchangeOAuthCallback(): Promise<ApiSession | null> {
    ...
    const response = await this.postAuth<{ session: ApiSession }>('session', {
      accessToken,
      refreshToken,
  ...
  private postAuth<T>(path: string, body: unknown): Promise<T> {
    return firstValueFrom(
      this.http.post<T>(`${environment.apiUrl}/auth/${path}`, body, {
        withCredentials: true,
      }),
    );
  }
